...
class Application
{
    ...
    function AViewModule($data = NULL)
    {
        //        echo __DIR__;
        if (is_array($data)) {
            extract($data);
        ...
        $_Action = $this->getAction();
        $_Param = $this->getParam();
        $layout = sprintf("%s/Module/%s/View/%s/%s.phtml", __DIR__, $this->getModule(), $this->getController(), $this->getAction());
        include_once $layout;
    }
    ...
    function AView($data = [], $theme = null)
    {
        ...
        $_Controller = $this->getController();
        $_Action = $this->getAction();
        $view = sprintf("View/%s/%s.phtml", $_Controller, $_Action);
        if ($theme) {
            $view = sprintf("theme/%s/%s/%s.phtml", $theme, $_Controller, $_Action);
        }
        include_once $view;
        echo Model\Content::DeCodeHTML();
    }
