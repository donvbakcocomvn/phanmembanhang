...
app.controller("pagesindexController", function($http, $scope) {
    ...
    $scope.SaveProduct = async  function() {
        await  $http.post("/apibe/SaveProduct/", $.param($scope._Product), headerFormData).then(function(res) {
...
app.controller("giohangCotroller", function($http, $scope) {
    ...
    $scope.ChuyenDonHang = function(username, orderCode) {
        ...
        $http.post("/apibe/chuyenDonHang/", $.param(data), headerFormData).then(function(res) {
    ...
    $scope.HuyDonHang = function(lydo, id) {
        ...
        $http.post("/apibe/HuyDonHang/", $.param(data), headerFormData).then(function(res) {
    ...
    $scope.LuuThongTinKhachHanh = function(_Tinh, _Huyen, Order) {
        ...
        $http.post("/apibe/saveInformationCustomer/", $.param(data), headerFormData).then(res => {
            console.log(res);
        });
    }
    ...
    $scope.DanhSachDonHangNhanVien = async  function(Params, pagesIndex, pagesNumber) {
        await $http.post("/apibe/GetOrdersByUserName/" + pagesIndex + "/" + pagesNumber + "/", $.param(Params), headerFormData).then(function(res) {
            $scope._DanhSachDonHang = res.data;
        });
    }
    $scope.DanhSachAllDonHang = async  function(Params, pagesIndex, pagesNumber) {
        await $http.post("/apibe/GetAllOrders/" + pagesIndex + "/" + pagesNumber + "/", $.param(Params), headerFormData).then(function(res) {
            $scope._DanhSachDonHang = res.data;
        });
    }
    ...
    $scope.DataThuTienDonHang = function(id) {
        ...
        var data = {"Id": id};
        $http.post("/apibe/DataThuTienDonHang/", $.param(data), headerFormData).then(function(res) {
//            $scope._OrderDetail = res.data;
            $scope.GetOrderDetail(id);
            window.alert(res.data.Mes);
        });
    ...
    $scope.DaNopTienVeCongTy = function(id) {
        ...
        var data = {"Id": id};
        $http.post("/apibe/DaNopTienVeCty/", $.param(data), headerFormData).then(function(res) {
//            $scope._OrderDetail = res.data;
            $scope.GetOrderDetail(id);
            window.alert(res.data.Mes);
        });
    ...
    $scope.XacNhanDonHang = function(id) {
        ...
        var data = {"Id": id};
        $http.post("/apibe/XacNhanDonHang/", $.param(data), headerFormData).then(function(res) {
//            $scope._OrderDetail = res.data;
            $scope.GetOrderDetail(id);
            window.alert(res.data.Mes);
        });
    ...
    $scope.TaoDonHang = function(_Tinh, _Huyen) {
        try {
            ...
            $http.post("/cart/mcart/taodonhang/", $.param(data), headerFormData).then(function(res) {
                $scope.getcarts();
...
app.controller("diachiController", function($scope, $rootScope, $http, $routeParams) {
    $scope._Tinh = '32';
    $scope._Huyen = '33';
